...
def process_video():
    ...
    # Convert the video data to a NumPy array for OCR
    image_array = cv2.imdecode(np.frombuffer(video_data, np.uint8), -1)

    # Use OCR to extract text from the image
    ocr_result = ocr_text(image_array)

    # Return the combined text output
    return jsonify({'prediction_result': prediction_result, 'ocr_result': ocr_result})
